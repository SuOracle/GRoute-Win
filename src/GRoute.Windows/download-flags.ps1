...
$dir = Join-Path $PSScriptRoot 'Assets\Flags'
New-Item -ItemType Directory -Force -Path $dir | Out-Null

$base = 'https://flagcdn.com/w160'
$codes = @(
...
$ok = 0
foreach ($c in $codes) {
    $out = Join-Path $dir "$c.png"
    Invoke-WebRequest -Uri "$base/$c.png" -OutFile $out -UseBasicParsing
    if (Test-Path $out) { $ok++ }
}

Write-Host "Downloaded $ok flags into $dir"
